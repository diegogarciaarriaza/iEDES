...
class RescatarVisitaAplazadaCommand extends ContainerAwareCommand{
    
    protected function configure(){
    
        $this->setName('visitas:rescate_visita_aplazada')
            ->setDescription('Convierte en rescatable las visitas aplazadas')
    ...
    protected function execute(InputInterface $input, OutputInterface $output){
        ...
        $query = $em->getRepository("iedeswebBundle:Visitas")->createQueryBuilder('v')
              ->where("v.fechaVisita < '" . $date->format('Y-m-d') . "'" .
                      ...
                      " and v.resultadovisitas = 9 and v.rescatar = 0")
