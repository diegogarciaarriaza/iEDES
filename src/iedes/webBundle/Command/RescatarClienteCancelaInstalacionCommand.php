...
class RescatarClienteCancelaInstalacionCommand extends ContainerAwareCommand{
    
    protected function configure(){
    
        $this->setName('visitas:rescate_cliente_cancela_instalacion')
            ->setDescription('Convierte en rescatable las instalaciones canceladas por el cliente')
    ...
    protected function execute(InputInterface $input, OutputInterface $output){
        ...
        $query = $em->getRepository("iedeswebBundle:Visitas")->createQueryBuilder('v')
              ->where("v.fechaVisita < '" . $date->format('Y-m-d') . "'" .
                      " and v.activo = 1 and v.contacto = 0 and v.resultadovisitas = 10 and v.rescatar = 0")
        ...
        $contenedor->get("email_service")->sendmail( 
              ...
              $to,
              'Rescates por Cliente Cancela Instalacion',
              $msg);
    }
}
