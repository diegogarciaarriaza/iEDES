...
class RescatarNoRespuestaTelefonoCommand extends ContainerAwareCommand{
    
    protected function configure(){
    
        $this->setName('visitas:rescate_visita_no_respuesta_telefono')
            ->setDescription('Convierte en rescatable las visitas sin respuesta al teléfono')
    ...
    protected function execute(InputInterface $input, OutputInterface $output){
        ...
        $date = $contenedor->get('utils_service')->changeDate(null, "-7 days");
        ...
        $query = $em->getRepository("iedeswebBundle:Visitas")->createQueryBuilder('v')
              ->where("v.fechaVisita < '" . $date->format('Y-m-d') . "'" .
                      ...
                      " and v.resultadovisitas = 12 and v.rescatar = 0")
        ...
        $contenedor->get("email_service")->sendmail( 
              ...
              $to,
              'Rescates por Visita con No Respuesta al Teléfono',
              $msg);
    }
}
